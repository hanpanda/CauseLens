...
def findpath(nx_g, u, dgl_id_to_name):
    ...
    for v in nx_g.successors(u):
        v_path_length, v_path = findpath(nx_g, v, dgl_id_to_name)
...
def sum_effect_along_path(nx_g, dgl_id_to_name):
    for u in nx_g.nodes:
        # root node
        if nx_g.in_degree(u) == 0:
            findpath(nx_g, u, dgl_id_to_name)
     
    # add (ntype, id, score) to result
    candidates = []
    for u in nx_g.nodes:
        candidates.append({'ntype': nx_g.nodes[u]['ntype'],
                           'id': nx_g.nodes[u]['id'],
...
def causal_effect(run_dir, 
    ...
    logger.debug('Load mask features.')
    ...
    for i, label in enumerate(tqdm(labels)):
        ...
        try:
            ...
            # debug
            # logger.debug('label: {}. {}. {}. Exception: {}.'.format(lbl['timestamp'], lbl['cmdb_id'], lbl['failure_type'], e))
            rsts[cnt]['cand'] = []
            cnt += 1
            continue
        feats = {ntype: feats_all[ntype][i] for ntype in feats_all.keys()}  
        node2score = {v: scores[i] for i, v in enumerate(anomalous_nodes)}
        nx_g = get_possible_subgraph(g, anomalous_nodes)        
        ...
        for u_, v_ in nx_g.edges:
            ...
            if score_type == 'kde':
                ...
                score_counter = compute_anomaly_score_by_kde(v_ntype, feat_1.reshape((1, -1)), estimator, label['timestamp'], dist_stats)[0][v]
            else:
                feat_2 = data_stats['mean'][v_ntype][v].detach()
                score_counter = compute_anomaly_score(v_ntype, feat_1, feat_2, label['timestamp'], dist_stats)[0][v]
            score = node2score[v]
            ...
            # debug
            # if lbl['failure_type'] in ['k8s容器网络延迟', 'network_delay']:
            #     logger.debug('time: {}. label: {}. {}. u_{}: {}. v_{}: {}. score: {}. score_counter: {}. diff: {}. dist: {}. mean: {}. std: {}.'
            #                  .format(lbl['timestamp'], lbl['failure_type'], lbl['cmdb_id'], u_ntype, u, v_ntype, v, score, score_counter, score_diff, dist, mean, std))
        ...
        if postprocess == 'sum':
            candidates = sum_effect_along_path(nx_g, dgl_id_to_name)
        elif postprocess == 'max':
            candidates = max_effect(nx_g)
...
def unify_results(individual_ad_rsts, causal_rsts, individual_coef, dgl_id_to_name, num_cands=10, agg=False, agg_duration=2):
...
def compute_rca_metrics(rsts_all, groundtruths_all, failure_type=None, agg=False):
    ...
    top_acc = {i: 0 for i in range(1, 11)}
    avg_rank = 0
    skipped = 0
...
def compute_anomaly_score(ntype, feat_1, feat_2, timestamp, dist_stats):
    # (num_nodes)
    error = euclidean(feat_1, feat_2).numpy()
    
    delta = 3
    timestamp = pd.Timestamp(timestamp) - pd.Timedelta(minutes=delta)
...
def compute_anomaly_score_for_all_samples(run_dir, 
                                          feats_all, 
                                          feats_preds, 
                                          labels, 
                                          score_type, 
    ...
    if score_type == 'kde':
        ...
        dist_stats = compute_distance_statistics_for_windows(run_dir=run_dir, dist_type=dist_type, score_type=score_type, window=window)
    ...
    for ntype in ntypes:
        ...
        for i, label in enumerate(tqdm(labels)):
            ...
            if score_type == 'train_and_pred':
                ...
                score, mean, std, dist = compute_anomaly_score_by_kde(ntype, feat, estimators, label['timestamp'], dist_stats)
    ...
    for i, label in enumerate(tqdm(labels)):
        ...
        # if score_type == 'pred_and_mean' or score_type == 'kde':
        if score_type == 'train_and_pred':
            # AIOps22
            examples = {'2022-03-20 08:49:00': [20, 26], 
                        '2022-03-24 19:10:00': [36, 30, 26, 15, 10, 37, 16],
                        '2022-03-21 13:01:00': [31, 13, 10]}
            
            # TT
            examples = {'2023-01-30 11:53:00': [172, 184, 181, 193, 5, 273, 2, 266]}
            
            # myTT
            ntype = 'api'
            examples = {'2024-04-06 05:03:00': [11, 91, 80],
                        '2024-04-06 04:54:00': [171, 181, 183, 193],
                        '2024-04-06 04:55:00': [(124, 11), (119, 11), (216, 18), (113, 10), (114, 10), (107, 10)],
                        
                        # ts-auth network
                        '2024-04-06 05:02:00': [(91, 9), (80, 9)],
                        '2024-04-06 05:03:00': [(91, 9), (80, 9)],
                        '2024-04-06 05:04:00': [(91, 9), (80, 9)],
                        
                        '2024-04-25 07:04:00': [(257, 23), (256, 23), (255, 23), ],
                        '2024-04-25 08:11:00': [(157, 12), (156, 12), (152, 12), (146, 12)],
                        '2024-04-25 13:11:00': [(163, 13), (161, 13), (162, 13)],
                        '2024-04-25 12:11:00': [(270, 24), (264, 24), (258, 24)],
                        
                        # exception
                        '2024-04-25 16:28:00': [(15, 1), (11, 1)],
                        '2024-04-25 16:51:00': [(15, 1), (11, 1)],
                        '2024-04-25 17:21:00': [(15, 1), (11, 1)],
                        
                        '2024-04-25 17:06:00': [(237, 21), (242, 21)],
                        '2024-04-25 17:05:00': [(288, 26), (291, 26)],
                        '2024-04-25 16:51:00': [(194, 16), (20, 2)]}
            
            # ntype = 'pod'
            # examples = {'2024-04-25 12:35:00': [25],}
            
            try:
                if label['timestamp'] in examples.keys():
                    logger.debug('---------------------timestamp: {}---------------------'.format(label['timestamp']))
                                        
                    for item in examples[label['timestamp']]:
                        if isinstance(item, tuple):
                            nid, pod = item
                            pod_feat = feats_all['pod'][i][pod].numpy()
                        else:
                            nid = item
                            pod_feat = None
                        
                        feat = feats_all[ntype][i][nid].numpy()
                        feat_pred = feats_preds[ntype][i][nid].numpy()
                        raw_score_mean = means[ntype][i][nid]
                        raw_score_std = stds[ntype][i][nid]
                        raw_score = dists[ntype][i][nid]
                        score = scores[ntype][i][nid]
                        feat_mean = -1
                        if data_stats is not None:
                            feat_mean = data_stats['mean'][ntype][nid].numpy()
                        ...
                        logger.debug(
                            ...
                            .format('nid', nid, 
                                    ...
                                    'feat_pred', feat_pred, 
                                    'mean', feat_mean, 
                                    'mean dist', raw_score_mean, 
                                    'std dist', raw_score_std, 
                                    'score', score,
                                    'dist', raw_score))
            except Exception as e:
                logger.error(e)
