...
class GraphAE(nn.Module):
    def __init__(self, conv_type, node_in_feats, in_feats, hidden_feats, num_enc_layers, num_dec_layers, num_heads, bidirected=False):
        super().__init__()
    ...
    def forward(self, graph, x, device):
        ...
        x_proj = th.concat(x_proj)
        
        x_hid = self.encoder(graph, x_proj)
        x_dec = self.decoder(x_hid)
        
        x_out = {}
        x_dec_list = th.split(x_dec, x_dims, dim=0)
    ...
    def __init__(
        ...
        if embedding:
            ...
            for node_type in node_in_feats.keys():
                self.emb_layer_dict[node_type] = nn.Embedding(num_embeddings=num_nodes_dict[node_type], 
    ...
    def forward(self, g, X, get_attention=False):
        ...
        if self.embedding:
            for ntype, emb_layer in self.emb_layer_dict.items():
                emb_dict[ntype] = emb_layer(th.arange(self.num_node_dict[ntype]).to(g.device)).to(g.device)
