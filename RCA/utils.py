...
def mahalanobis(u, v, cov):
    delta = u - v
    if delta.dim() == 1:
        m = th.dot(delta, th.matmul(th.pinverse(cov), delta))
        return th.sqrt(m)
    elif delta.dim() == 2:
        ms = []
        for slice in delta:
            m = th.sqrt(th.dot(slice, th.matmul(th.pinverse(cov), slice)))
            ms.append(m.item())
        return th.tensor(ms)
...
def tsne_visualization(X: np.array, labels: dict, title: str='t-SNE Visualization of Tensor Data with Labels', path='images/tmp.png'):
