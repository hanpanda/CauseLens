...
class myGATConv(nn.Module):
    ...
    def __init__(
        ...
        if self._embedding:
            self.fc_emb_src = nn.Linear(
                emb_dim, out_feats * num_heads, bias=bias
            )
            self.fc_emb_dst = nn.Linear(
                emb_dim, out_feats * num_heads, bias=bias
            )
        else:
        ...
        self.fc_edge_1 = nn.Linear(
            etype_feats, out_feats * num_heads, bias=bias
        )
        self.fc_edge_2 = nn.Linear(
            edge_feats, out_feats * num_heads, bias=bias
        )
            ...
            if isinstance(feat, dict):
                ...
                if self._embedding:
                    ...
                    for ntype, node_emb in emb_dict.items():
                        feat_emb_src[ntype] = self.fc_emb_src(node_emb).repeat(graph.batch_size, 1).view(-1, self._num_heads, self._out_feats)
                        feat_emb_dst[ntype] = self.fc_emb_dst(node_emb).repeat(graph.batch_size, 1).view(-1, self._num_heads, self._out_feats)
...
class myDotGATConv(nn.Module):
    ...
    def forward(self, graph, feat, emb_dict, get_attention=False):
        ...
        for ntype, nfeat in h.items():    
            if self._embedding:
                feat_q[ntype] = self.fc_q(emb_dict[ntype]).repeat(graph.batch_size, 1).view(
                ...
                feat_q[ntype] = self.fc_q(nfeat).view(
                    -1, self._num_heads, self._out_feats
                )
            ...
            feat_v[ntype] = self.fc_v(nfeat).view(
                -1, self._num_heads, self._out_feats
            )
        ...
        if self.use_edge_feats:
            for etype, efeat in graph.edata['feat'].items():
                feat_edge[etype] = self.fc_e(efeat).view(
                    -1, self._num_heads, self._out_feats
                )
        else:
            for etype in graph.canonical_etypes:
                num_edges = graph.num_edges(etype=etype)
                feat_edge[etype] = th.zeros(
                    (num_edges, self._num_heads, self._out_feats)
                ).to(device)
        ...
        if self.res_fc is not None:
            for ntype in rst.keys():
                resval = self.res_fc(h[ntype]).view(
                    h[ntype].shape[0], -1, self._out_feats
                )
                rst[ntype] = rst[ntype] + resval
